...
class backwardDifferentiation:
	...
	def numericalSolution(self,problem):
		...
		for k in range(0,self.M):
			self.numericalAns[k+1,1:self.N] = np.dot(np.linalg.inv(A) , (self.numericalAns[k,1:self.N]+self.timeStep*f))

	# def test(self,problem):
	# 	I=np.eye(self.N)
	# 	C=np.eye(self.N,k=1)+np.eye(self.N,k=-1)
	# 	A = ((1+2.0*self.r)*I-self.r*C)
	# 	f=problem.f*np.ones(self.N)

	# 	self.numericalAns=np.array(self.numericalAns)
	# 	self.preciseAns=np.array(self.preciseAns)
	# 	print(self.numericalAns[1,1:self.N+1])
	# 	print(np.dot(np.linalg.inv(A),self.numericalAns[0,1:self.N+1].transpose()+self.timeStep*f))
	...
	def showPicture(self,problem):
		...
		figure=plt.figure()
		ax=Axes3D(figure)
		figure.canvas.set_window_title('backward differentiation')
