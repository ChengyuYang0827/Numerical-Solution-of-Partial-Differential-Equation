...
class CrankNicolson:
	...
	def numericalSolution(self,problem):
		...
		ALeft=(1+self.r)*I-self.r/2.0*C
		ALeftInv=np.linalg.inv(ALeft)
		ARight=(1-self.r)*I+self.r/2.0*C
		...
		for k in range(0,self.M):
			self.numericalAns[k+1,1:self.N]=np.dot(ALeftInv,np.dot(ARight,self.numericalAns[k,1:self.N])+self.timeStep*f)
	...
	def showPicture(self,problem):
		...
		figure=plt.figure()
		ax=Axes3D(figure)
		figure.canvas.set_window_title("crank nicolson")
